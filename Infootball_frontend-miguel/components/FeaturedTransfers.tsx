...
export default function FeaturedTransfers() {
  ...
  useEffect(() => {
    const fetchTransfers = async () => {
      ...
      try {
        ...
        setTransfersData(transformedTransfers);
      } catch (err) {
  ...
  const featuredTransfers = transfersData.filter(t => t.featured).slice(0, 3);
  const regularTransfers = transfersData.filter(t => !t.featured).slice(0, 3);
  ...
  if (error || transfersData.length === 0) {
    return (
      <section className="mb-12">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
        ...
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-bold mb-2">No hay fichajes disponibles</h3>
          <p className="text-gray-400">Los fichajes aparecerán aquí cuando estén disponibles</p>
        </div>
      </section>
    );
  }
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex-1 text-center">
                    <p className="flex items-center justify-center gap-1">
                      {transfer.playerImage.startsWith('http') ? (
                        <img src={transfer.playerImage} alt={transfer.player} className="w-6 h-6 rounded-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      ) : (
